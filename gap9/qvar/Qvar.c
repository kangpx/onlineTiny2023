...
AT_HYPERFLASH_EXT_ADDR_TYPE Qvar_L3_Flash = 0;
...
static float accuracy_score(void);
...
int main(void)
{
    ...
    //                         construct model                          //
    //------------------------------------------------------------------//
    int ConstructorErr = QvarCNN_Construct();
    if (ConstructorErr)
    {
...
static float accuracy_score(void)
{
    ...
    for (unsigned int i=0; i<N_TEST_SAMPLE; i++)
        if (y_test_pred[i] == y_test_real[i])
            correct_cnt += 1;
    return ((float)correct_cnt) / ((float)N_TEST_SAMPLE);
}
