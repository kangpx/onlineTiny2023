	...
	for (int l=0; l<Ho; l++) {
		...
		for (int c=0; c<Wo; c++) {
            ...
            /*if (Size==1 && c==0)
                for (int f=F; f<L; f++)
                    for (int i=0;i<3;i++)
                    printf("c=%d, %f, %f, %f\n", c, ((F16*)(ColBuff+Fx*f))[0], ((F16*)(ColBuff+Fx*f))[1], ((F16*)(ColBuff+Fx*f))[2]);
                    */
			...
			gap_waitbarrier(0);
					...
					F16V C0 = VBuff[2*i], C1 = VBuff[2*i+1];
                        ...
                        // F16 *filter = &(Filter[Line*W_In1+0]);
                        // F16 accumulate = (F16)0.0f;
                        // for (int i=0;i<W_In1;i++) 
                        //     accumulate += (filter[i] * ((F16*)ColBuff)[i]);
                        // accumulate += Bias[Line];
                        // Out[Line*Wo*Ho + l*Wo + c] = accumulate;
	                }
			gap_waitbarrier(0);
		}
		PosL += Sy;
...

{
    ...
    // printf("CoreId=%d, InFeat=%d, C=%d, F=%d, L=%d\n", CoreId, InFeat, C, F, L);
	int Iter = (L-F)*Fx;
	int PosL = 0;
	for (int l=0; l<Ho; l++) {
		int PosC = -PadL;
		for (int c=0; c<Wo; c++) {
			...
			int Off = -Lb - Min(PosC, 0);
			int Size = Rb-Lb;
            //if(CoreId==0 && InFeat==7)
            //    printf("%d, %d, %d, %d\n", Off+Lb, Lb, Size, Off+Rb-1);
			if (Size>=2) {
				if (Size&0x1) {
					for (int f=F; f<L; f++) {
						for (int i=0; i<Size/2; i++) *((int *)(ColBuff+Fx*f+Off+2*i+Lb)) = *((int *)(In+f*W*H+PosL*W+2*i+Lb));
...
*/
			...
			gap_waitbarrier(0);
	                for (int Line=First; Line<Last; Line++) {
                        ...
                        //     accumulate += (filter[i] * ((F16*)ColBuff)[i]);
                        // accumulate += Bias[Line];
                        // Out[Line*Wo*Ho + l*Wo + c] = MaxF(accumulate, (F16)0.0);
	                }
			gap_waitbarrier(0);
		}
		PosL += Sy;
...

{
	...
	*/
	...
	int Tail = 2*((W_In1+3)/4);
	F16 * __restrict__ ColBuff1 = ColBuff + 2*Tail;
	((int *)ColBuff)[Tail-1] = 0;
	...
	for (int l=0; l<Ho; l++) {
		...
		for (int c=0; c<(Wo/2); c++) {
			...
			if (Iter1&0x1) {
				((short int *)(ColBuff+F*FS))[Iter1-1]=0;
				((short int *)(ColBuff1+F*FS))[Iter1-1]=0;
			}
			int Lb = Max(PosC, 0), Rb = Min(PosC+Fx, W);
			...
			int OffC1 = -Lb1 - Min(PosC+Sx, 0);
					...
					for (int j=Tb; j<Db; j++) {
						...
						for (int i=Lb1; i<Rb1; i++)
							((short int *)(ColBuff1+(j+OffL)*InFeat*Fx+(i+OffC1)*InFeat+F))[Iter-1] = ((short int *)(In+j*W*InFeat + i*InFeat+F))[Iter-1];
					}
                        } else {
				for (int j=Tb; j<Db; j++) {
					for (int i=Lb; i<Rb; i++)
			...
			F16 *pOut1 = Out+l*Wo*OutFeat + (2*c+1)*OutFeat+First;
				...
				F16 *pC0 = pC, *pC1 = pC0+W_In1, *pC2 = pC1+W_In1, *pC3 = pC2+W_In1;
				pC=pC3+W_In1;
	                        F16V S0 = (F16V){*pBias,0.0}, S4=S0; pBias++;
	                        F16V S1 = (F16V){*pBias,0.0}, S5=S1; pBias++;
	                        F16V S2 = (F16V){*pBias,0.0}, S6=S2; pBias++;
	                        F16V S3 = (F16V){*pBias,0.0}, S7=S3; pBias++;
				F16 *pIn = ColBuff;
				F16 *pIn1 = ColBuff1;
	                        for (int i=0; i<(W_In1/2); i++) {
					F16V V0 = *((F16V *)pIn), V1 = *((F16V *)pIn1), C0 = *((F16V *)pC0), C1 = *((F16V *)pC1), C2 = *((F16V *)pC2), C3 = *((F16V *)pC3);
					S0 += V0*C0; S4 += V1*C0;
					S1 += V0*C1; S5 += V1*C1;
					S2 += V0*C2; S6 += V1*C2;
					S3 += V0*C3; S7 += V1*C3;
					pIn+=2; pIn1+=2; pC0+=2; pC1+=2; pC2+=2; pC3+=2;
	                        }
				if (W_In1&0x1) {
					F16V V0 = (F16V){*pIn,0.0}, V1 = (F16V){*pIn1,0.0}, C0 = *((F16V *)pC0), C1 = *((F16V *)pC1), C2 = *((F16V *)pC2), C3 = *((F16V *)pC3);
					S0 += V0*C0; S4 += V1*C0;
					S1 += V0*C1; S5 += V1*C1;
					S2 += V0*C2; S6 += V1*C2;
					S3 += V0*C3; S7 += V1*C3;
				}
				*(pOut0+4*Line+0) = S0[0]+S0[1]; //pOut0++;
				*(pOut0+4*Line+1) = S1[0]+S1[1]; //pOut0++;
				*(pOut0+4*Line+2) = S2[0]+S2[1]; //pOut0++;
				*(pOut0+4*Line+3) = S3[0]+S3[1]; //pOut0++;
				*(pOut1+4*Line+0) = S0[0]+S0[1]; //pOut0++;
				*(pOut1+4*Line+1) = S1[0]+S1[1]; //pOut0++;
				*(pOut1+4*Line+2) = S2[0]+S2[1]; //pOut0++;
				*(pOut1+4*Line+3) = S3[0]+S3[1]; //pOut0++;
	                }
			for (int i=4*(IterOut/4); i<IterOut; i++) {
				F16 *pIn = ColBuff;
				F16 *pIn1 = ColBuff1;
	                        F16V S0 = (F16V){*pBias,0.0}, S1=S0; pBias++;
	                        for (int i=0; i<(W_In1/2); i++) {
					F16V V0 = *((F16V *)pIn), V1 = *((F16V *)pIn1), C0 = *((F16V *)pC);
					S0 += V0*C0;
					S1 += V1*C0;
					pIn+=2; pIn1+=2; pC+=2;
				}
				if (W_In1&0x1) {
					F16V V0 = (F16V){*pIn,0.0}, V1 = (F16V){*pIn1,0.0}, C0 = *((F16V *)pC);
					S0 += V0*C0;
					S1 += V1*C0;
				}
				*(pOut0+i) = S0[0]+S0[1]; // pOut0++;
				*(pOut1+i) = S1[0]+S1[1];
			}
			gap_waitbarrier(0);
		}
...

{
	...
	*/
	...
	for (int l=0; l<Ho/2; l++) {
		int PosC = -PadL + First*Sx;
		int Tb = Max(PosL, 0), Db = Min(PosL+Fy, H);
		int OffL = -Tb - Min(PosL, 0);
		int Tb1 = Max(PosL+Sy, 0), Db1 = Min(PosL+Sy+Fy, H);
		int OffL1 = -Tb1 - Min(PosL+Sy, 0);
		// for (int c=0; c<Wo; c++) {
			...
			if (Iter>=2) {
					...
					for (int i=Lb; i<Rb; i++) {
							...
							((int *)(ColBuff+(j+OffL)*InFeat*Fx+(i+OffC)*InFeat))[f] = ((int *)(In+j*W*InFeat + i*InFeat))[f];
                                        	for (int j=Tb1; j<Db1; j++)
							((int *)(ColBuff1+(j+OffL1)*InFeat*Fx+(i+OffC)*InFeat))[f] = ((int *)(In+j*W*InFeat + i*InFeat))[f];
					}
                                if (Iter&0x1)
					for (int i=Lb; i<Rb; i++) {
						for (int j=Tb; j<Db; j++)
						...
						for (int j=Tb1; j<Db1; j++)
							((short int *)(ColBuff1+(j+OffL1)*InFeat*Fx+(i+OffC)*InFeat))[Iter-1] = ((short int *)(In+j*W*InFeat + i*InFeat))[Iter-1];
					}
                        } else
				for (int i=Lb; i<Rb; i++) {
					for (int j=Tb; j<Db; j++)
					...
					for (int j=Tb1; j<Db1; j++)
						ColBuff1[(j+OffL1)*InFeat*Fx+(i+OffC)*InFeat] = In[j*W*InFeat + i*InFeat];
				}
			PosC += Sx;

			F16 *pBias = Bias;
			F16 *pC = Filter;
			F16 *pOut0 = Out+(2*l+0)*Wo*OutFeat + c*OutFeat;
			F16 *pOut1 = Out+(2*l+1)*Wo*OutFeat + c*OutFeat;
	                for (int Line=0; Line<IterOut/4; Line++) {
				F16 *pC0 = pC, *pC1 = pC0+W_In1, *pC2 = pC1+W_In1, *pC3 = pC2+W_In1;
				pC=pC3+W_In1;
	                        F16V S00 = (F16V){*pBias,0.0}, S01 = S00; pBias++;
	...
	if (Ho&0x1) {
		...
		//for (int c=0; c<IterW/2; c++) {
			...
			F16 *pBias = Bias;
			F16 *pC = Filter;
			F16 *pOut0 = Out+(l)*Wo*OutFeat + c*OutFeat;
	                for (int Line=0; Line<IterOut/4; Line++) {
				F16 *pC0 = pC, *pC1 = pC0+W_In1, *pC2 = pC1+W_In1, *pC3 = pC2+W_In1;
				pC=pC3+W_In1;
	                        F16V S00 = (F16V){*pBias,0.0}; pBias++;
...

{
	...
	for (int l=0; l<Ho; l++) {
		int PosC = -PadL;
		int Tb = Max(PosL, 0), Db = Min(PosL+DFy, H);
		int OffLBuffY = (Tb==0)?Max(0, gap_mulsN(-PosL+Dy-1, InvDy, Prec)):0;
		int OffLInY = OffLBuffY?(Dy*OffLBuffY+PosL):0;
		for (int c=0; c<Wo; c++) {
			for (int i=0; i<(Iter1/2); i++) ((int *)(ColBuff+F*FS))[i]=0;
			if (Iter1&0x1) ((short int *)(ColBuff+F*FS))[Iter1-1]=0;
