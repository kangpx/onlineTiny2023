...
def create_parser():
    ...
    parser.add_argument('--mode', default="generate_at_model", choices=["accuracy", "performance", "generate_at_model"],
                        help="Script mode")
    ...
    parser.add_argument('--ne16', default=False, action="store_true",
                        help="Use NE16 HW accelerator in GAP9")
...
def nntool_build_model(model_path, quant_opts, already_quantized=False):
    ...
    if not already_quantized and not quant_opts["scheme"] == "FLOAT":
        # No need to collect statistics if we want to deploy float or the model is already quantized
        from train_tf import get_dataset
        x_train, x_test, y_train, y_test = get_dataset()
        calibration_data = x_train[:100]
        def representative_dataset():
            for data in tqdm(calibration_data, desc="Calibration:"):
